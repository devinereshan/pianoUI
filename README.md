...
#### `setSize(size)`
Sets the size of the piano interface. Accepts an array specifying the `width` and `height` of the piano. Both values must be provided whenever setting `size`. They may be any valid string for specifying the dimensions of a `<div>`. For example:
...
Attaches a callback function to one of the internal events fired by a piano object. Events fired are `'noteOn'` and `'noteOff'`.


## Events

Piano objects fire two distinct events internally: `'noteOn'` and `'noteOff'`. A callback function can be attached to either event with the `on(eventName, callback)` method. For example:

```javascript
let piano = new PianoUI.Piano("#piano-container");

piano.on('noteOn', (event) => {
  console.log(event);
...
piano.on('noteOff', myNamedFunction);
```
...
```
indicating that the event was fired from a `'mouseDown'` event from the user on key `24`, with a velocity of `127`.

The possible `eventSource` values are:
* `'mouseDown'` - the mouse was clicked over the specified note, triggering a `'noteOn'` event.
* `'mouseOver'` - the mouse was clicked outside of the specified note and dragged into it, triggering a `'noteOn'` event.
* `'mouseUp'` - the mouse was released inside the current note, triggering a `'noteOff'` event.
* `'mouseOut'` - the mouse left the current note while still being held down, triggering a `'noteOff'` event.
* `'external'` - the event was fired by a method call (`setKeyActive(...)` or `setKeyInactive(...)`), rather than a user interaction with the interface.
